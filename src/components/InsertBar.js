...
export default function InsertBar(props) {
    ...
    const addCrush = (e) => {
        ...
        cy.add([
            ...
            { group: 'edges', data: { id: currentUserUni + crushUni, source: currentUserUni, target: crushUni } }
        ]);
    ...
    return (
        <>
            <form id="crush-form" onSubmit={(e) => {addCrush(e)}}>
                <label className="label" htmlFor="add-crush">Add a Crush: </label>
                <input type="text" id="add-crush" name="add-crush" placeholder='UNI of your crush'
                value={crushUni} onChange={(e) => props.setCrushUni(e.target.value)}/>
                <input id="submit-btn" type="submit" value="Add" />
            </form>
            {error && <Alert id="error" className='text-center' key="danger" variant="danger">{error}</Alert>}
        </>
    )
}